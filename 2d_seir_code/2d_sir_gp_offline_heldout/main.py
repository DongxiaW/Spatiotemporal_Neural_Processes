...
beta_epsilon_all = beta_epsilon.reshape(-1,2)

beta = np.repeat(np.expand_dims(np.linspace(1.2, 3.9, 10),1),3,1)
epsilon = np.repeat(np.expand_dims(np.linspace(0.25, 0.65, 3),0),10,0)
beta_epsilon = np.stack([beta,epsilon],-1)
...
def train(training_iterations,x_train,y_train, patience = 50):
...
def mae_plot(mae, i):
    ...
    plt.savefig('mae_plot_seed%d.pdf' % (i))
...
ytrain_set, ytrain_mean, ytrain_std = seir(num_days,beta_epsilon_train,num_simulations)
y_train = ytrain_set.reshape(-1,100)
x_train = np.repeat(beta_epsilon_train,num_simulations,axis =0)
...
for seed in range(3): #3
    ...
    ypred_testset.append(y_pred_test)

    test_mae = MAE(y_pred_test,y_test)
    mae_testset.append(test_mae)
    ...
    ypred_allset.append(y_pred_all)
    mae_matrix, mae = MAE_MX(y_pred_all, y_all)
    
    
    maemetrix_allset.append(mae_matrix)
    mae_allset.append(mae)
    print('All MAE:',mae)
    mae_plot(mae_matrix, selected_mask,seed)
