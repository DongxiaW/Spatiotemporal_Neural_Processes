...
def NLL(pred_mu, pred_cov, y):
        y = y[:,1:,...]
#         print("shape of y before reshape: ", y.shape)
        y = y.reshape(y.shape[0], -1)
#         print("shape of y after reshape: ", y.shape)
#         print("shape of pred_cov before reshape: ", pred_cov.shape)
        pred_cov = pred_cov.reshape(pred_cov.shape[0], -1)
#         print("shape of pred_cov after reshape: ", pred_cov.shape)
#         print("shape of pred_mu before reshape: ", pred_cov.shape)
        pred_mu = pred_mu.reshape(pred_mu.shape[0], -1)
#         print("shape of pred_mu after reshape: ", pred_cov.shape)
        pred_std = torch.sqrt(pred_cov)
#         print("shape of pred_std: ", pred_std.shape)
#         print("shape of pred_mu: ", pred_mu.shape)
...
#         self.repr_encoder = REncoder(x_dim+conv_outDim, r_dim) # (x,y)->r
        ...
        self.mean_encoder_in_decoder = nn.Linear(32, 32)
        self.cov_encoder_in_decoder = nn.Linear(32, 32)
...
#         outputs = outputs.reshape(seq_len, -1)
#         print("flattened outputs shape: ", outputs.shape)
        outputs_mean = self.mean_encoder_in_decoder(outputs).reshape(seq_len, 1, 2, 32, 32)
        outputs_cov = self.cov_m(self.cov_encoder_in_decoder(outputs)).reshape(seq_len, 1, 2, 32, 32)
#         print("shape of outputs_mean: ", outputs_mean.shape)
#         print("shape of outputs_cov: ", outputs_cov.shape)
        

            
        return outputs_mean, outputs_cov
...
#         print("shape of z_mu_all: ", self.z_mu_all.shape)
        ...
        outputs_cov = []
        for target in x_t:
            output_mean, output_cov = self.decoder(target, self.z_mu_all, self.z_logvar_all)
            outputs_mean.append(output_mean)
            outputs_cov.append(output_cov)
        outputs_mean = torch.stack(outputs_mean, dim=0)
        outputs_cov = torch.stack(outputs_cov, dim=0)
...
def test(x_train, y_train, x_test):
    with torch.no_grad():
        ...
        output_list_mean = None
        output_list_cov = None
        for i in range (len(x_test)):
        #           zsamples = sample_z(z_mu, z_logvar) 
            output_mean, output_cov = dcrnn.decoder(x_test[i:i+1].to(device), z_mu, z_logvar)
            if output_list_mean is None:
                output_list_mean = output_mean.cpu().unsqueeze(0).detach()
                output_list_cov = output_cov.cpu().unsqueeze(0).detach()
            else:
                output_list_mean = torch.vstack((output_list_mean, output_mean.cpu().unsqueeze(0).detach()))
                output_list_cov = torch.vstack((output_list_cov, output_cov.cpu().unsqueeze(0).detach()))
    
    return output_list_mean.numpy(), output_list_cov.numpy()
...
#         y_target  = y_train
        ...
        y_pred_mean, y_pred_cov = dcrnn(x_t, x_c, y_c, x_ct, y_ct)
        
#         print("shape of y_pred: ", y_pred.shape)
#         print("shape of y_t: ", y_t.shape)

        train_loss = NLL(y_pred_mean, y_pred_cov, y_t) + dcrnn.KLD_gaussian()
        nll_loss = NLL(y_pred_mean, y_pred_cov, y_t)
        ...
        opt.step()
         ...
         #val loss
        y_val_pred_mean, y_val_pred_cov = test(torch.from_numpy(x_train).float(),torch.from_numpy(y_train).float(),
                      torch.from_numpy(x_val).float())
        val_loss = NLL(torch.from_numpy(y_val_pred_mean).float(),torch.from_numpy(y_val_pred_cov).float(),torch.from_numpy(y_val).float())
        #test loss
        y_test_pred_mean, y_test_pred_cov = test(torch.from_numpy(x_train).float(),torch.from_numpy(y_train).float(),
                      torch.from_numpy(x_test).float())
        test_loss = NLL(torch.from_numpy(y_test_pred_mean).float(),torch.from_numpy(y_test_pred_cov).float(),torch.from_numpy(y_test).float())

        if t % n_display ==0:
            print('train loss:', train_loss.item(), 'nll:', nll_loss.item(), 'kld:', kld_loss.item(), flush=True)
...
# in meanstd, x_train is single theta, y_train is the corresponding ground truth
def calculate_score(x_train, y_train, search_data_x, sample_time = 30):
    x_train = torch.from_numpy(x_train).float()
    y_train = torch.from_numpy(y_train).float()
    search_data_x = torch.from_numpy(search_data_x).float()
    ...
    with torch.no_grad():
        ...
        score_list = []
        for theta in search_data_x:
            mean_list = []
            cov_list = []
            # sample 30 times for std
            for i in range(sample_time):
                search_mean, search_cov = dcrnn.decoder(theta, z_mu, z_logvar)
                mean_list.append(search_mean[-1:,:,0:1,...].cpu().detach().numpy())
                cov_list.append(search_cov[-1:,:,0:1,...].cpu().detach().numpy())
            y_search_mean =  np.stack(mean_list)
            y_search_mean = y_search_mean.reshape((sample_time, -1))
            y_search_cov = np.stack(cov_list)
            y_search_cov = y_search_cov.reshape((sample_time, -1))
            mean = np.mean(y_search_mean,0)
            cov = np.cov(y_search_mean.T)          
            E_H_list = []

            for k in range (len(y_search_mean)):
                E_H = multivariate_normal(y_search_mean[k], y_search_cov[k],allow_singular=True).entropy()
                E_H_list.append(E_H)

            E_H = np.mean(np.array(E_H_list))
            H = multivariate_normal(mean, cov,allow_singular=True).entropy()
            score = H - E_H + 120
...
for i in range(10): #8
    ...
    train_losses, val_losses, test_losses, z_mu, z_logvar = train(20000,x_train,y_train,x_val, y_val, x_test, y_test,500, 2000) #20000, 5000
    y_pred_test, _ = test(torch.from_numpy(x_train).float(),torch.from_numpy(y_train).float(),
                      torch.from_numpy(x_test).float())
#     print("y_pred_shape: ", y_pred_test.shape)
    ...
    y_pred_all, _ = test(torch.from_numpy(x_train).float(),torch.from_numpy(y_train).float(),
...
#     print("shape of y_pred_all: ", y_pred_all.shape)
    ...
    reward_list = calculate_score(x_train, y_train, search_data_x)
...
#     print('reward_list:',reward_list)
    selected_ind = np.argpartition(np.array(reward_list), -5)[-5:]
    x_train = np.concatenate((x_train, [search_data_x[i] for i in selected_ind]), axis=0)
    y_train = np.concatenate((y_train, [search_data_y[i] for i in selected_ind]), axis=0)
    
    search_data_x = np.array([e for i, e in enumerate(search_data_x) if i not in selected_ind])
    search_data_y = np.array([e for i, e in enumerate(search_data_y) if i not in selected_ind])
...
np.save("final_ypred_testset_final_20by8_theta_stacked_y_stnp_64_rdim_2_theta_active_learning.npy", np.array(ypred_testset))
np.save("final_maemetrix_allset_20by8_theta_stacked_y_stnp_64_rdim_2_theta_active_learning.npy", np.array(maemetrix_allset))
np.save("final_mae_allset_20by8_theta_stacked_y_stnp_64_rdim_2_theta_active_learning.npy", np.array(mae_allset))
np.save("final_mae_testset_20by8_theta_stacked_y_stnp_64_rdim_2_theta_active_learning.npy", np.array(mae_testset))
torch.save(dcrnn.state_dict(), "final_5_seq_20by8_theta_stacked_y_stnp_64_rdim_2_theta_active_learning.pt")
