...
#         print("shape of y_search: ", y_search.shape)
        ...
        z_mu_search, z_logvar_search = data_to_z_params(x_train.to(device),y_search.to(device), calc_score = True)
...
#     print("shape of y_pred_all: ", y_pred_all.shape)
    ...
    x_train = np.concatenate((x_train, [search_data_x[i] for i in sort_ind[-5:]]), axis=0)
    y_train = np.concatenate((y_train, [search_data_y[i] for i in sort_ind[-5:]]), axis=0)
    
    search_data_x = [search_data_x[i] for i in sort_ind[:-5]]
    search_data_y = [search_data_y[i] for i in sort_ind[:-5]]
